...
const DetalhesClienteScreen = ({ route, navigation }: any) => {
  ...
  // --- TRATAMENTO DO CÓDIGO DO CLIENTE ---
  // Garante que codCli seja sempre a string do código, mesmo que 'cliente' seja um objeto
  ...
  // Estados para separação financeira
        ...
        try {
          ...
          if (salvos) {
            ...
            const filtrados = lista.filter((p: any) => {
              // Compara o código independente de como o pedido foi salvo
              const pCod =
  ...
  useEffect(() => {
    const carregarDetalhes = async () => {
      try {
        setLoading(true);
        // Usando codCli para garantir que a URL da API fique correta
        const response = await api.get(
          `/api/getdetalhamentocliente?cliente=${codCli}&loja=${loja}`,
        );
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        <Button
          ...
          icon="cart-plus"
          onPress={() =>
            navigation.navigate("SelecaoTabela", { cliente, loja, vendedorId })
          }
          style={[
            styles.btnNovoPedido,
            temAtraso && { backgroundColor: "#d32f2f" },
          ]}
      ...
      <List.Item
        ...
        onPress={() =>
          navigation.navigate("PedidosOffline", {
            cliente: codCli,
            loja: loja,
          })
        }
